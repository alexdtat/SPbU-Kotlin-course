...
fun generatePlotTimeOnSize(
    ...
    val style = scaleYContinuous(format = "{} ms") + scaleXContinuous(breaks = sizesToTimeMap.keys.toList()) +
        ...
        subtitle = "Time dependence on list size. There are $parallelingResourcePercentage% (+1)" +
            " of list's size ${sortingModeText.lowercase()} for each list."
    )
...
fun generateTimeOnParallelingResource(
    ...
    val data = mapOf(
        "$sortingModeText count" to parallelBranchesToTimeMap.keys.toList(),
